 ...
 *    
 * [내부 클래스의 특징]
 * - 내부 클래스는 외부 클래스의 멤버를 마치 자기 자신처럼
 *   사용이 가능함. ==> 데이터 값을 변경시킬 수도 있음.
 *   외부 클래스 멤버의 접근지정자가 private 접근지정자 일지라도
 *   접근이 가능함.
...
class Outer { // 외부 클래스
	int su1 = 50; // 바뀌지 않으려면 final 붙여서 상수화 시켜주면된다.
	private int su2 = 34;
...
public class Inner_02 {
	...
	public static void main(String[] args) {
		...
		// 중첩 클래스 객체 생성 방법
		// 1.외부 클래스 객체가 먼저 생성이 되어야 한다.

		Outer outer = new Outer();

		// 2.내부 클래스 객체 생성
		// 형식) 외부클래스명.내부클래스명 참조 변수는 = 외부클래스참조변수.new 내부 클래스 생성자();
